...
export class ExamSchedulerComponent implements OnInit {
    ...
    constructor(@Inject(LOCALE_ID) locale: string, private appService: ExamSchedulerService, private dateAdapter: DateAdapter, private dialog: MatDialog) {
        this.locale = locale;
    ...
    ngOnInit(): void {
        this.appService.getAll().subscribe((evaluations: Evaluation[]) => {
    ...
    segmentClicked(action: string, segment: SchedulerViewHourSegment): void {
        const dialogRef = this.dialog.open(EvaluationFormDialogComponent, {
            ...
            data: {
                ...
                endTime: new Date(segment.date.getTime() + 60 * 60 * 1000) // +1h default
            }
        });
        ...
        dialogRef.afterClosed().subscribe((result: Evaluation | undefined) => {
            if (result) {
                ...
                // Ako želiš odmah slati na backend, možeš ovde dodati servisni poziv, npr:
                // this.appService.create(result).subscribe(...);
            }
        });
    }
